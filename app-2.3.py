...
def ticker_for_alpha_vantage(ticker, exchange):
    ...
    if exchange == "US":
        ...
        # Alpha Vantage uses "BSE:SYMBOL" format, but we'll assume ticker is already in symbol format
        return f"BSE:{ticker}"
    return ticker
...
def get_stock_news(ticker):
    try:
        ...
        news = stock.news
        if not news:
            return fetch_external_news(ticker)
        formatted_news = []
        for item in news[:5]:
            ...
            if 'providerPublishTime' in item:
                try:
                    date_str = pd.to_datetime(item['providerPublishTime'], unit='s').strftime('%Y-%m-%d %H:%M')
                except:
                    pass
...
def fetch_external_news(ticker):
    ...
    if not news_api_key:
        ...
        return []
    try:
        url = f"https://newsapi.org/v2/everything?q={ticker}+stock&apiKey={news_api_key}"
        ...
        return [{
            ...
            "date": pd.to_datetime(article.get("publishedAt")).strftime('%Y-%m-%d %H:%M'),
            "summary": article.get("description", "")[:250] + "..."
        } for article in articles[:5]]
    except Exception as e:
        logger.error(f"External news error for {ticker}: {str(e)}")
        return []
...
def predict_future_prices(df, days=30):
    ...
    try:
        ...
        logger.warning(f"ARIMA failed: {str(e)}. Using Linear Regression")
        df = df.copy()
...
def calculate_portfolio_value(holdings, exchange):
    ...
    for item in holdings:
        try:
            ...
            shares = item["shares"]
            stock = yf.Ticker(ticker)
            # Check if market is open; if not, try fetching the last available data
            history = stock.history(period="1d")
            if history.empty:
                # Try fetching the last available close price (up to 5 days back)
...
def add_alert(client, user_id, ticker, target_price, condition, exchange):
    ...
    alert = {
        "user_id": user_id,
        "ticker": ticker,  # Store base ticker without suffix
        "exchange": exchange,
...
def calculate_risk_metrics(holdings, exchange, benchmark="^GSPC"):
    if not holdings:
        return None
    if exchange == "NSE":
        benchmark = "^NSEI"  # Nifty 50 for NSE
    elif exchange == "BSE":
        benchmark = "^BSESN"  # BSE Sensex
    else:
        benchmark = "^GSPC"  # S&P 500 for US
    portfolio_values = []
    ...
    with tabs[1]:
        ...
        if holdings:
            ...
            if value_data:
                ...
                # Create pie chart only if df_portfolio has data
